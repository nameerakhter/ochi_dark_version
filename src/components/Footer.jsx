export default function Footer() {
  const socials = {
    ...
    L: ['202-1965 W 4th Ave\nVancouver, Canada', '30 Chukarina St\nLviv, Ukraine'],
    E: 'hello@ochi.design',
    M: ['Home', 'Services', 'Our work', 'About us', 'Insights', 'Contact us']
  };

  return (
    <div className="w-full h-screen bg-zinc-900 p-20 text-white flex gap-5">
      <div className='w-1/2 h-full font-["Neue Montreal Regular"] flex flex-col justify-between'>
        <div className="heading">
          <h1 className="uppercase text-[7vw] font-semibold leading-none -mb-15">
            EYE-
          </h1>
          <h1 className="uppercase text-[7vw] font-semibold leading-none -mb-10">
            OPENING
          </h1>
        </div>
        <div className="ochi">
      ...
      <div className='w-1/2 h-full font-["Neue Montreal Regular"] flex flex-col justify-between'>
        <h1 className="uppercase text-[6vw] font-semibold leading-none -mb-10">PRESENTATIONS</h1>
        <div className="flex justify-between">
          <div>
            <h3 className="mb-8">S:</h3>
            {socials.S.map(social => (
              <a key={social} href="#" className="block mb-2 underline">{social}</a>
            ))}
          </div>
          <div>
            <h3 className="mb-8">L:</h3>
            {socials.L.map(location => (
              <p key={location} className="whitespace-pre-line mb-4">{location}</p>
            ))}
          </div>
          <div>
            <h3 className="mb-8">E:</h3>
            <a href={`mailto:${socials.E}`} className="underline">{socials.E}</a>
          </div>
          <div>
            <h3 className="mb-8">M:</h3>
            {socials.M.map(item => (
              <a key={item} href="#" className="block mb-2 underline">{item}</a>
            ))}
          </div>
        </div>
      </div>
