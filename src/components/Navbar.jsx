...
export default function Navbar() {
  ...
  return (
    <div className="font-['Neue Montreal Regular'] fixed z-[999] w-full text-white shadow-md">
      <div className="flex items-center justify-between p-4">
        ...
        {/* Navigation Items */}
        <div
          className={`absolute left-0 top-16 w-full flex-col items-center gap-4 text-white transition-all duration-300 sm:relative sm:left-auto sm:top-0 sm:flex sm:w-auto sm:flex-row sm:gap-10 sm:bg-transparent ${
            menuOpen ? 'block' : 'hidden'
          }`}
        >
          {['Services', 'Work', 'About', 'Insights', 'Contact'].map(
            (item, index) => (
              <a
                key={index}
                className={`block cursor-pointer text-lg hover:text-gray-700 sm:inline-block sm:text-base ${
                  index === 4 ? 'sm:ml-32' : ''
                }`}
              >
                {item}
